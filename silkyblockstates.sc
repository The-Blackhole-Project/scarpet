...
_enchant(player) -> (
    ...
    if(!(item ~ '_axe$' || item ~ '_pickaxe$' || item ~ '_hoe$' || item ~ '_shovel$') || nbt:'Enchantments[{id:"maismp:silkyblockstate"}]',
    ...
    nbt_map:'Enchantments' += {'id' -> 'maismp:silkyblockstate'};
    nbt = encode_nbt(nbt_map);
    nbt:'Enchantments[{id:"maismp:silkyblockstate"}].lvl' = nbt('1s');
