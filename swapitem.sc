...
swap(fromEntity, fromSlot, toEntity, toSlot) -> (
    if(!fromEntity || !toEntity,
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f Look an entity to run this command.')))
    );
    if(!player() ~ 'permission_level' && (isOtherPlayer(player(), fromEntity) || isOtherPlayer(player(), toEntity)),
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f You can\'t swap items to another player.')))
    );
    if(!isValid(fromEntity) || !isValid(toEntity),
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f The entity you\'re looking at is not a valid entity.')))
    );
    if(trade(fromEntity) || trade(toEntity),
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f You can\'t swap items that are villager trades to villager')))
    );
    if(fromEntity == toEntity && fromSlot == toSlot,
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f You can\'t swap items on the same slot of the same entity.')))
    );
    toItem = toEntity ~ ['holds', toSlot] || [null, null, null];
    fromItem = fromEntity ~ ['holds', fromSlot] || [null, null, null];
    if(toItem == fromItem,
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f You can\'t swap two identical items.')))
    );
    ...
    if(!success,
        ...
        return(print(player(), format('&8[&em&ba&6i&7SMP&8]&f You can\'t swap this two items.')))
    );
